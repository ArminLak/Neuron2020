% This code takes animal name and session and will make simple plots
...
postAlign = ((sample_rate/downsampleScale)*abs(start)+300):((sample_rate/downsampleScale)*(abs(start))+800);
...
if isfield(photoMdata, 'AnalogIn_4_dF_F0')
...
for istim = Stimz
    ...
    performance(c) = nanmean (TrialTimingData(TrialTimingData(:,2)==istim,9));
...
for iChan = 1:NChan
    ...
    plot(downsample(TimeStamps,10),smooth(downsample(DeltaFoverF(iChan,:),10)),'color', [0.25 0.25 0.25])
    hold on
    % line([90 90], [-2 4])
